...
The follow-on GitHub Check Runs mapper has now tested one bounded caller-attested provider observation. The current [roadmap](../development/roadmap.md) freezes further transport and live-acquisition work until the local vertical CLI path has a real external consumer. Preserve the current payload, envelope, subject, principal, execution-context, and enforcement bindings without adding a scheduler, credentials store, broad provider matrix, or automatic external mutation.
