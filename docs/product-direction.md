...
- non-interactive `init`, `onboard`, `diff`, `render`, and `check`;
...
- explicitly replace a complete existing target through the accepted
  onboarding boundary after selected guidance has been represented in
  canonical rules and while its complete replacement plan is current; or
