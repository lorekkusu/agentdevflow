...
The npm product is published as `agentdevflow@0.1.0-beta.1` but is not stable for 1.0. Its installed bin qualifies non-interactive `init`, read-only `check` and `diff`, exact approved `render`, and explicit-observation `doctor`. Publication remains protected by a manual exact-commit workflow, the `npm-publish` Environment, and an npm OIDC trusted publisher without a stored GitHub publishing secret. ADR 0004 accepts the initial beta discovery, support, licensing, command, exit, output, and release boundaries while retaining documented beta migration authority.

The smallest current outcome is one reproducible init-to-check/diff/render vertical path, bounded local doctor composition, exact-root project defaults, bounded JSON schema version 1, and an allowlisted package verified both offline and from the public registry. Normal-user beta feedback remains outstanding.
...
Current Node.js 24 release-candidate verification passes strict type checking, build, and 394 tests with zero failures, skips, or todos; the repository audit passes over 220 text files. The selected V1 suite passes 294 tests from 36 files. The exact 118-entry tarball installs both offline and from the public registry and completes all five commands through the installed npm bin. Registry advisory matching reports zero production vulnerabilities, installed dependency queries report no preinstall, install, or postinstall scripts, and npm verifies the dependency signatures and published package provenance described in the evidence. See [beta release-candidate evidence](../evidence/beta-release-candidate.md) and [initial beta publication evidence](../evidence/initial-beta-publication.md).

The public npm registry now serves exact version `0.1.0-beta.1` with the qualified source commit, integrity, contents, registry signature, and provenance. At release-preparation commit `96c253c`, a bounded full-history disclosure audit covered 43 reachable commits, 223 historical paths, and 488 distinct blobs without finding a publication blocker through its documented pattern, path, blob, metadata, or external-state checks. It does not replace human review or a dedicated secret scanner. Public-repository hardening enables private vulnerability reporting, Dependabot security updates, secret scanning, push protection, full-SHA Actions pinning, an active no-bypass `main` ruleset requiring the six selected checks and squash-only pull requests, and a second ruleset rejecting identity-based remote branch prefixes. The `npm-publish` environment is restricted to `main`, requires an explicit approval from `lorekkusu`, and contains no secret; self-approval and administrator bypass remain possible and must not be represented as independent review.
...
- **Current:** beta hardening, exact-commit qualification, bounded disclosure preflight, public visibility, protected pull-request closure, purpose-based branch-name enforcement, exact-version npm publication, registry installation, bootstrap-token revocation, and OIDC trusted-publisher configuration are complete. Merge the token-free workflow closure before collecting normal-user feedback.
...
Beta release hardening, exact-commit publication, and registry verification are complete. Version `0.1.0-beta.1` was published from commit `e4b3014` through the protected workflow and verified against its candidate digest, provenance, tarball contents, and clean registry installation. The one-time GitHub secret and npm bootstrap token are removed, and the package is bound to the repository's OIDC trusted publisher. The current gate is this token-free repository closure. Git tags, GitHub Releases, later npm versions, package-setting changes, and scope expansion remain separately gated.
