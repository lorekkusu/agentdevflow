...
plan binds current canonical sources, observed bytes, generated bytes, and file
actions. No classification store, proposal file, second writer, or second
approval path is added. Ordinary blocked diffs still suppress foreign bytes;
the exact authorized replacement diff shows the complete before and after
content.
...
`StagedRendererAdapter` is planning-only. Existing targets have one of five
outcomes:

- create an absent target;
- adopt exact generated bytes;
- replace through the current bounded equivalent-content authorization;
- replace exact reviewed unmanaged bytes through the onboarding authorization;
- abort on ambiguity, foreign ownership, or drift.

The onboarding path permits a complete existing-target replacement after
selected content has been represented in canonical rules and the replacement
is visible in the normal current plan. This is implemented as the exact
per-target replacement input described above. It does not add managed regions
or a second approval path.
