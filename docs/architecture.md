...
The experimental local CLI composes explicit repository, configuration, and lock paths into that planner and the existing command services. Init generates the bounded local revision-1 intent, validates provider dispositions through a read-only workspace, and exclusively creates only an absent configuration; it does not write provider outputs or the lock. Check and diff remain read-only. With no lock, planning binds exact adoption or proven lossless import into the same complete snapshot shown by diff. Render requires approval of that snapshot, rereads configuration, reanalyzes provider bytes, opens the mutation-capable workspace, replans, requires the same digest again, and then invokes the sole forward-convergent render service. Option names, output, exit behavior, discovery, and npm packaging remain private. This is an application boundary over the compiler and renderer contracts, not a second planner, diff engine, or provider-output writer.
