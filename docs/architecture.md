...
The [private compiled-policy consumer spike](development/private-compiled-policy-consumer.md) is the narrow downstream composition of exact project-document compilation with explicit caller-supplied capability observations, internally derived manifest export, strict canonical trace decoding, and existing replay. It accepts neither caller-selected compiler output nor a manifest package. It is path-free, unpublished, and does not choose transitions, acquire or create evidence, persist state, or stabilize the private trace representation. It proves composability, not an independently usable product outcome.
...
The beta local CLI composes repository, configuration, and lock paths into that planner and the existing command services. Init generates the bounded local revision-1 intent, validates provider dispositions through a read-only workspace, and exclusively creates only an absent configuration; it does not write provider outputs or the lock. Check and diff remain read-only. With no lock, planning binds exact adoption or proven lossless import into the same complete snapshot shown by diff. Render requires approval of that snapshot, rereads configuration, reanalyzes provider bytes, opens the mutation-capable workspace, replans, requires the same digest again, and then invokes the sole forward-convergent render service. ADR 0004 and the beta CLI contract define the public prerelease behavior; internal module names, TypeScript types, and service interfaces remain private. This is an application boundary over the compiler and renderer contracts, not a second planner, diff engine, or provider-output writer.

When an interrupted apply leaves a managed path at its exact target while the base lock remains authoritative, the render path can reconstruct the originally approved all-before plan. Reconstruction changes only create, update, or delete observations that are provably at the matching target state, removes only their corresponding ownership-conflict diagnostics, and recomputes the complete snapshot. The original approval must match that snapshot, and convergent apply still requires every actual path to be at its before or target digest. This is not adoption, foreign-drift repair, or a journal.
