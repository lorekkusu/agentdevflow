...
Do not create an ADR for a local refactor, an easily reversible tool preference, an unresolved brainstorm, or a temporary experiment. Keep reversible current choices in `docs/development/tooling.md`, candidates in `docs/development/roadmap.md`, and observations in `docs/evidence/`.
...
6. Verify that no prompt, transcript, private instruction, identity, local path, credential, or unsupported estimate is present.
